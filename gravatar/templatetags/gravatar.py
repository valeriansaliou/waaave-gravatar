...
def _wrap_img_tag(url, size):
    return '<img src="%s"%s alt="" height="%s" width="%s"/>' % \
...
def _get_gravatar_id(email):
    return md5(email.lower().strip()).hexdigest()


@register.simple_tag
def gravatar_for_email(email, size=None, rating=None, default=True):
    """
    ...
    parameters = [p for p in (
        ('d', GRAVATAR_DEFAULT_IMAGE if default else '404'),
...
def gravatar_img_for_email(email, size=None, rating=None):
    ...
    return _wrap_img_tag(gravatar_url, size)
